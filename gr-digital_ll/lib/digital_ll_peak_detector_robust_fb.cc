...
digital_ll_peak_detector_robust_fb_sptr
digital_ll_make_peak_detector_robust_fb (float threshold_factor_rise, 
    float threshold_factor_fall, 
    int look_ahead, 
    float alpha)
{
	return digital_ll_peak_detector_robust_fb_sptr (new digital_ll_peak_detector_robust_fb
...
digital_ll_peak_detector_robust_fb::digital_ll_peak_detector_robust_fb 
    (float threshold_factor_rise, 
    float threshold_factor_fall, 
    int look_ahead, 
    float alpha)
	: gr_sync_block ("peak_detector_robust_fb",
...
int
digital_ll_peak_detector_robust_fb::work (int noutput_items,
			...
			gr_vector_void_star &output_items)
{
  ...
  while(i < noutput_items) {
    ...
    if (d_avg != d_avg){
      // If d_avg != d_avg, then it is NaN. We don't want this! Set to 0
      d_avg = 0;
    }
    
    // Print the meta data to standard-error
    if(VERBOSE){
        fprintf(stderr, "d_avg: %+.4f \n", d_avg);
        fprintf(stderr, "thres: %+.4f \n", d_avg*d_threshold_factor_rise);
    }
