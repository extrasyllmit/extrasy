...
from digital_ll import parse_frame_file # where is this from?
from digital_ll import receive_path_gmsk
from digital_ll import tdma_logger
from digital_ll import tdma_types_to_ints # where is this from?
...
from mac_ll import base_slot_manager_static
from mac_ll import mobile_slot_manager_static
...
class my_top_block(gr.top_block):
    def __init__(self, modulator, demodulator, options, ll_logging, dev_log, start_tb_time, time_cal_timeout, start_controller_time):
        ...
        if self.tdma_mac_sm.is_base():
            ...
            manage_slots = base_slot_manager_static(types_to_ints = tdma_types_to_ints,
                                                    options = options,
            ...
            manage_slots = mobile_slot_manager_static(types_to_ints = tdma_types_to_ints,
                                                      options = options,
                                                      tdma_mac = self.tdma_mac_sm,
                                                    )
        ...
        if self.tdma_mac_sm.is_base():
            ...
            self.connect(self.source, self.rx_time_tag_shifter, self.scheduled_mux,self.rx_path)
        ...
        # handle mobile node specific setup    
            ...
            self.connect(self.source, self.rx_time_tag_shifter,self.scheduled_mux,self.beacon_consumer)
    ...
    def do_time_calibration(self):
        ...
        if self.tdma_mac_sm.is_base():
            ...
            self.dev_log.info("starting time calibration over %f seconds",
                                 self.cal_time)    
...
#    normally this would be in transmit path add option
    ...
    base_slot_manager_static.add_options(parser,expert_grp)
    mobile_slot_manager_static.add_options(parser,expert_grp)
    ...
    for key in node_defaults:
        ...
        if key in opt_list:
            ...
            #dev_log.info('Updating option default: %s from node_defaults', key)
