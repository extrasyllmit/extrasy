...
from mac_ll import Sequential_Pattern_Agent
from mac_ll import tdma_base_sm
from mac_ll import tdma_controller
from mac_ll import tdma_mobile_sm
from mac_ll import Sequential_Pattern_Agent_Wrapper
from mac_ll import base_rl_agent_protocol_manager
from mac_ll import mobile_rl_agent_protocol_manager
...
class my_top_block(gr.top_block):
    def __init__(self, modulator, demodulator, options, ll_logging, dev_log, start_tb_time, time_cal_timeout, start_controller_time):
        ...
        options.sequential_pattern_order = [int(x) for x in options.sequential_pattern_order.split(',')]
        ...
        if self.tdma_mac_sm.is_base():
            ...
            agent = Sequential_Pattern_Agent(options.sequential_pattern_order)
                

            
            agent_wrapper = Sequential_Pattern_Agent_Wrapper(agent, 
                                                             epoch_len, 
                                                             1,
                                                             change_delay, 
                                                             mobile_ids,
...
#    log_config["loggers"]["developer"]["level"] = known.log_level
    ...
    parser.add_option("", "--sequential-pattern-order", type="string", default="0, 1", help="order to cycle through patterns")
...
#            assert False
    ...
    dev_log.info("saving agent log file to %s", abs_agentlog)
    dev_log.info("saving database log file to %s", abs_dblog)  
